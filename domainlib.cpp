...
int nodal2dziff(int N, double *u, int *d, int *siz, int *nd, int sign, int verb)
/* Simple Ziff quick union/find alg for labeling +/- nodal domains on 2D grid.
   Single thread only.

   Inputs: u, a real NxN contiguous array of doubles.
           N, the square side length (positive integer).
	   sign: if 0 count domains of both signs, if >0 just +ve, <0 just -ve.
	   verb: verbosity (0,1,2,3).
   Outputs:
           d, an int NxN contiguous array containing domain numbers
              where domains are regions in u with same sign, connected along
              the usual four orthogonal directions. Numbers are in 1,..,nd.
...
*/
{
  ...
  if (verb) printf("nodal2dziff: N=%d, n=%d\n",N,n);
  ...
  for (int y=0; y<N; ++y) // cluster all nei w/ same sign (ignore sign arg)
    for (int x=0; x<N; ++x) {
...
int perc2d(int N, double *u, int *d, int* siz, int* nd, int nh, double *hran,
	   double *h0, int verb)
/* 
   Select percolation threshold on gridded 2D real data, from a list of
   ...
   Inputs: u, a real NxN contiguous array of doubles.
	   ...
	   verb, verbosity (0,1,2,3).
   Outputs:
           d, an int NxN domain number array (as in nodal2dziff), at threshold
...
{
  int n=N*N;    // # grid pts
  if (verb) printf("perc2d: N=%d, n=%d, h: [%g,%g] %d levels\n",N,n,hran[0],hran[1],nh);
  ...
  // join up entire min y line, and max y line
  for (int x=0; x<N; ++x) {
    qunion(&ptr[0],0,x);          // y=0 join to site 0
    qunion(&ptr[0],n-1,x+n-N);    // y=N-1 join to size n-1
  }
  ...
  while (!perc && b>=0) {               // loop bins in descending order...
    *h0 = hran[0] + b*wid;              // lower end of this bin
	...
	if (u[i]<*h0 || (b<nh && u[i]>*h0+wid)) printf("%d fail: u=%g\n",i,u[i]);
      int y = (int)(i/N);               // get x,y coords of i
      int x = i-y*N;
